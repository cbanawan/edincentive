 ...
 */
class TbWizard extends \yii\bootstrap\Widget
{
	// Tab placements.
	...
	/**
	 * @var array the options for the tabs/nav container.
	 */
	public $navOptions = [];
	...
	public function run()
	{
        ...
        echo Html::tag('div', $this->placement === self::PLACEMENT_BELOW ? $content . $tabs : $tabs . $content, $this->htmlOptions);
		...
		$this->view->registerJs("jQuery('#{$id}').bootstrapWizard({$options});");

		foreach ($this->events as $name => $handler) {
			$handler = Json::encode($handler);
			$this->view->registerJs("jQuery('#{$id}').on('{$name}', {$handler});");
		}
	}
	...
	protected function normalizeTabs($tabs, &$panes, &$i = 0)
	{
		...
		foreach ($tabs as $tab) {
			...
			if (isset($tab['items'])) {
				...
				$panes[] = Html::tag('div',  $content, $paneOptions);
